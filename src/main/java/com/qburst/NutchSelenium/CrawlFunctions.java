...
public class CrawlFunctions extends NutchCrawler {
    ...
    public Path[] generateSegment(Configuration conf) throws IOException {
        ...
        Path[] segment = generator.generate(crawldbPath, segmentDirPath, 2, (Integer) dbObject.get(Constants.OUTLINK_URL_NUM), currentTime);
        return segment;
    }
    ...
    public void fetchURL(Configuration conf, Path[] segment) throws IOException {
        ...
        if (segment != null) {
            for (int i = 0; i < segment.length; i++) {
                int num_threads = 2;
                fetcher.fetch(segment[i], num_threads);

                ParseSegment parseSegment = new ParseSegment(conf);
                parseSegment.parse(segment[i]);

                contentParse(conf, segment[i]);
                getOutLinks(conf, segment[i]);
                parseForXPath(conf, segment[i]);
            }
    ...
    public void contentParse(final Configuration conf, Path segment) throws IOException {
        ...
        try {
            while (reader.next(key, HtmlContent)) {
                ...
                logger.warn(result);
                htmlFile = new File(dirPath.toString(), result + ".html");
                htmlFile.createNewFile();*/
                htmlFile = new File(dirPath.toString(), fileNum + "_" + "htmlContent.html");
                fileNum++;
                htmlFile.createNewFile();
    ...
    public void parseForXPath(Configuration conf, Path segment) throws IOException {
        ...
        SequenceFile.Reader parseReader = new SequenceFile.Reader(fs, parseContent, conf);
        Text key = new Text();
        ...
        try {
            while (reader.next(key, outLinkContent)) {
                ...
                for (int i = 0; i < outLinks.length; i++) {
                    String outLinkUrl = outLinks[i].getToUrl();
    ...
    public void addUrlFilter() {
        ...
        if (dbObject.get(Constants.FILTER_URL)!=null /*|| dbObject.get(Constants.SITE_URL) != null*/) {
            ...
            urlRegex.append("(");
            for(int i=0; i<urlsToFilter.size(); i++) {
                urlRegex = urlRegex.append(urlsToFilter.get(i) +"/*"+"|");
            }
